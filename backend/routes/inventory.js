...
// GET / - Display all inventory items for the current user
router.get(`/inventories-${STUDENT_ID}`, async (req, res) => {
    try {
        ...
        res.render('inventory/inventories', {
            title: 'My Inventory - CloudKitchen Pro',
            isLoggedIn: true,
            user: req.user,
...
// GET - Display add inventory form
router.get(`/add-${STUDENT_ID}`, (req, res) => {
    res.render('inventory/add-inventory', {
        title: 'Add Inventory Item - CloudKitchen Pro',
        isLoggedIn: true,
        user: req.user,
        STUDENT_ID: req.app.locals.STUDENT_ID || STUDENT_ID,
        STUDENT_NAME: req.app.locals.STUDENT_NAME || STUDENT_NAME,
...
router.post(`/add-${STUDENT_ID}`, inventoryValidationMiddleware, async (req, res) => {
    // If validation middleware attached validation errors, render add-inventory with errors
    ...
    try {
        ...
        // Redirect to inventory listing page
        res.redirect(`/inventory/inventories-${STUDENT_ID}?userId=${req.user.userId}`);
    } catch (error) {
...
// GET - Display edit inventory form
    ...
    try {
        ...
        if (!inventoryToEdit) {
            return res.status(404).render('errors/404', {
                title: 'Inventory Item Not Found - CloudKitchen Pro',
                userId: req.user && req.user.userId ? req.user.userId : '',
                STUDENT_ID: req.app.locals.STUDENT_ID || STUDENT_ID,
                STUDENT_NAME: req.app.locals.STUDENT_NAME || STUDENT_NAME
            });
        ...
        // Render edit form with the selected inventory item if found
        res.render('inventory/edit-inventory', {
            title: 'Edit Inventory Item - CloudKitchen Pro',
            isLoggedIn: true,
            user: req.user,
            STUDENT_ID: req.app.locals.STUDENT_ID || STUDENT_ID,
            STUDENT_NAME: req.app.locals.STUDENT_NAME || STUDENT_NAME,
...
// POST - Update existing inventory item
router.post(`/edit-${STUDENT_ID}/:id`, inventoryValidationMiddleware, async (req, res) => {
    ...
    // If validation middleware attached validation errors, render edit-inventory with errors
    if (req.validationErrors && req.validationErrors.length > 0) {
    ...
    try {
        ...
        // If no inventory item was found to update, show 404
        if (!updatedInventory) {
            return res.status(404).render('errors/404', {
                title: 'Inventory Item Not Found - CloudKitchen Pro',
                userId: req.user && req.user.userId ? req.user.userId : '',
        ...
        // Redirect back with success message (use inventoryId if present)
        res.redirect(`/inventory/inventories-${STUDENT_ID}?userId=${req.user.userId}&msg=${encodeURIComponent(updatedInventory.ingredientName + 
            ' (' + updatedInventory.inventoryId + ') updated successfully')}`);
    } catch (error) {
...
router.post(`/delete-${STUDENT_ID}`, async (req, res) => {
    try {
        ...
        const toDeleteInventory = await Inventory.findOne({ _id: id });
        const deletedInventory = await Inventory.findByIdAndDelete(id);

        // If no inventory item was found to delete, show 404
        if (!deletedInventory) {
            return res.status(404).render('errors/404', {
                title: 'Inventory Item Not Found - CloudKitchen Pro',
                userId: req.user && req.user.userId ? req.user.userId : '',
    ...
    // Redirect back to inventory list after successful deletion with toast message
    res.redirect(`/inventory/inventories-${STUDENT_ID}?userId=${req.user.userId}&msg=${toDeleteInventory.ingredientName}(${toDeleteInventory.inventoryId}) deleted successfully`);
    }
    catch (error) {
